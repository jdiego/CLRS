...
Let x be a node with two children. In an inorder tree walk, the nodes in x's left subtree immediately precede x and the nodes in x's 
right subtree immediately follow x. Thus, x's predecessor is in its left subtree, and its successor is in its right subtree.

Let s be x's successor. Then s cannot have a left child, for a left child of s would come between x and s in the inorder walk. 
(It's after x because it's in x's right subtree, and it's before s because it's in s' left subtree.) If any node were to come 
between x and s in an inorder walk, then s would not be x's successor, as we had supposed. 
Symmetrically, x's predecessor has no right child.

So, if node x in a BST has two children, then its successor s is the left most node in x's right subtree and hasn't a left child. 
And if node x in a BST has two children, then its predecessor s is the right most node in x's left subtree and hasn't a right child.
